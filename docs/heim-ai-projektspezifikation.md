...
| `Local-AI-Voice-Orchastrator` (GitHub: daswoody) (**NEU in v1.6**) | Monorepo: `orchestrator/` (FastAPI + LangGraph, 1.7+1.11), `stt-service/` (faster-whisper, 1.8), `tts-piper/` (Filler, 1.9), `tts-xtts/` (Hauptstimme, 1.10), `tts-breeze/` (Test-Engine Breeze TTS 2, eigener Deploy, v1.17), `tts-breeze-cpp/` (Breeze-TTS-2.cpp mit CUDA, eigener Deploy, v1.18), später `admin-frontend/` (1.7c) — siehe 4.14 | 🚧 1.7–1.11 code-seitig fertig (v1.7), VM-Validierung offen |
...
[Voice-Orchestrator] (eigener Python-Service mit LangGraph)
   ...
   ▼
[TTS-Service] ─── Piper (Filler) + XTTS-v2 (Hauptantwort; Engine im Panel wählbar, v1.17;
                   weitere Engines per Engine-Vertrag, z. B. Qwen3-TTS, v1.20)
   ▼
...
- **XTTS ausschalten (NEU in v1.19):** Zusätzliche Zuweisung **„Aus"** (`off`) nur für XTTS — der Dienst entlädt Modell und Latents und beantwortet Synthesen mit 503, bis wieder eine Karte zugewiesen wird; gespeichert und wiederhergestellt wie jede Zuweisung. Der Container läuft weiter (kein Docker-Socket, siehe oben); den CUDA-Kontext des Prozesses (einige hundert MB) gibt erst ein Container-Neustart frei. Regeln im Orchestrator: nicht ausschaltbar, solange XTTS die aktive Hauptstimme ist (auch per `.env`-Default); ausgeschaltet weder aktivierbar noch probehörbar, keine neuen XTTS-Filler (vorhandene bleiben abspielbar); Rückfallebene bei Ausfall der aktiven Engine ist dann **Piper** statt XTTS. Anlass: Breeze-Test auf derselben Karte.
- **Engines nach dem Engine-Vertrag (NEU in v1.20):** Jede Vertrags-Engine (z. B. Qwen3-TTS, 4.14) erscheint automatisch als eigener Dienst `tts-<id>` mit CPU / GPU N / „Aus“. Ihr Modell läuft in einem eigenen Prozess: „Aus“ beendet ihn, das VRAM ist danach **komplett** frei — inklusive CUDA-Kontext, anders als bei XTTS. Gespeichert und wiederhergestellt wie jede Zuweisung; zusätzlich merkt sich der Orchestrator beim Ausschalten die vorherige Karte (`gpu_last_device_<dienst>`), damit „Aktivieren“ unter Sprachausgabe (4.14) die Engine dort wieder lädt und dafür andere Sprachausgaben auf derselben Karte entlädt.
...
- **Breeze TTS 2** (Test-Engine, v1.17/v1.18) — beide Server-Varianten laden ihr Modell beim Start auf eine feste Karte und kennen keinen Laufzeit-Wechsel; Karte per `BREEZE_GPU` in der jeweiligen Compose-Datei. Der offizielle PyTorch-Server (`docker-compose.breeze.yml`) belegt ~7,7 GB, Breeze-TTS-2.cpp (`docker-compose.breeze-cpp.yml`, Q8_0) ~4 GB — solange der Container läuft; deshalb eigene Deploys, die nur während Tests laufen.
...
**Interaktive Karten (v1.12.5, Nutzer-Wunsch „Karte, in der ich Flüge raussuchen kann"):** Karten sind nicht nur Anzeigen, sondern können **Mini-Tools** sein: Formulare, Eingabefelder, Buttons, kleine Rechner — typisch mit einem Button, der eine aus den Eingaben gebaute Such-URL (Google Flights, Bahn, …) in einem neuen Browser-Tab öffnet. Umsetzung: `show_card` bekommt ein `request`-Feld (Funktionsbeschreibung), das als Design-Brief an den `code-card`-Agenten geht (bei `request` darf `data` leer sein); die Tool-Beschreibung stellt ausdrücklich klar, dass interaktive Karten möglich sind (das Modell verneinte das vorher). Sandbox erweitert um `allow-forms allow-popups allow-popups-to-escape-sandbox` — Scripts/Formulare laufen isoliert wie bisher (weiterhin **kein** same-origin, kein Token-/Storage-Zugriff), Links öffnen einen normalen Tab. Der `code-card`-Auftrag erlaubt Inline-JS und verbietet externe Ressourcen/fetch (die Sandbox hat keinen verlässlichen Netzzugriff — externe Aktionen laufen über URL-Öffnung im Browser des Nutzers). In Chromium verifiziert (Playwright): Eingabe in der Karte → Klick → neuer Tab mit korrekt gebauter Flights-URL.

**Fähigkeiten im System-Prompt (v1.12.6, fünfter Praxistest):** Das lokale Modell lehnte interaktive Karten weiter ab, **ohne** `show_card` überhaupt aufzurufen — kleine Modelle beantworten „kann ich das?" aus ihrem Vorwissen („ich habe keine interaktive Oberfläche") und lesen die Tool-Beschreibungen dafür nicht. Deshalb hängt der Orchestrator die Fähigkeiten jetzt serverseitig an den System-Prompt jeder Session mit Karten-Kanal: `show_card` inkl. interaktiver Mini-Tools („lehne solche Wünsche NIEMALS ab") plus die Liste der aktiven Agenten mit ihren Beschreibungen (ohne den internen `code-card`). Der Admin-Charakter-Prompt bleibt unangetastet davor; die Tool-Beschreibungen bleiben die Detail-Referenz für den konkreten Aufruf.
...
**TTS-Fallback-Regel:** Kommt bis zur `done`-Nachricht kein Server-Audio im Turn, liest die App den Antwort-Text per Android-On-Device-TTS vor (in den Einstellungen abschaltbar). *Server-seitig davor (v1.17):* Scheitert eine andere Haupt-Engine als XTTS, bevor Audio geflossen ist, spricht XTTS — erst wenn auch das scheitert, greift die App-Regel.
...
**Voice-First-Antwortmodell (NEU in v1.13 — ersetzt „lange Antwort + Kurzfassung" als Normalfall):** Der Ziel-Use-Case ist: Spracheingabe → **kurze gesprochene Antwort** → **umfangreiche Informationen als Karte** aufs Gerät → nächste Spracheingabe kennt die Karten-Inhalte. Umsetzung: (1) Bei Spracheingaben (`voice_mode`) bekommt das LLM eine **Sprachmodus-Anweisung** in den System-Prompt — die generierte Antwort IST die Sprachantwort (1–3 natürliche Sätze, keine Formatierung/Listen/Emojis), alles Umfangreiche gehört per `show_card` in eine Karte mit kurzem Verweis. Die v1.10-Kurzfassung bleibt nur als **Sicherheitsnetz** für zu lang geratene Antworten. (2) **Gesprächsgedächtnis:** Die letzten `HISTORY_MAX_MESSAGES` (Default 12) Nachrichten der Konversation wandern in den LLM-Kontext — inklusive der **Karten-Daten** (`[Karte angezeigt: <Titel> - Daten: {…}]`, Layout-HTML wird als Kontext-Ballast weggelassen; Einzelnachrichten auf `HISTORY_MAX_CHARS_PER_MESSAGE` gekürzt). Damit kann das LLM auf „die Karte von eben" Bezug nehmen. Vorher wurde der Kontext pro Turn frisch aufgebaut — Folgefragen ohne Kontext waren eine offene Lücke.
...
**Stimmen-Verwaltung:** `GET/POST/DELETE /v1/admin/voices` + `POST /v1/admin/voices/{id}/sample` (WAV-Upload). Das Voices-Volume ist zwischen Orchestrator und XTTS-Service geteilt — ein Upload aus dem Panel landet direkt dort, wo das Voice-Cloning es erwartet (kein `docker cp`). Zuweisung an Nutzer über `default_voice_id`. `GET /v1/voices` (App-seitig, 4.13) bleibt unverändert. **Sample-Transkript (v1.17):** `voices.sample_text` (additive Migration) hält den exakten Wortlaut des Samples, den Breeze TTS 2 fürs Voice-Cloning braucht (XTTS nicht). Beim Upload schlägt Whisper (STT-Service) ihn vor — best effort, ein altes Transkript wird dabei verworfen, weil es zum neuen Sample nicht mehr passt; `POST /v1/admin/voices/{id}/transcribe` wiederholt das für vorhandene Samples, `PUT /v1/admin/voices/{id}` korrigiert. Die App-Schnittstelle zeigt das Feld nicht.
...
**Engine-Wahl pro Filler + Probehören (NEU in v1.15):** Praxisproblem: XTTS scheiterte bei einzelnen Fillern sporadisch, und das fiel erst auf, wenn der betroffene Filler zufällig im Realtime-Talk gespielt wurde. Zwei Ergänzungen im Panel:
- **Engine pro Filler wählbar** (`fillers.engine`, additive Migration, Default `xtts`): `xtts` spricht in der Nutzerstimme (kein Stimmbruch, braucht Voice-Sample und VRAM), `piper` nutzt die feste Piper-Stimme — robust, GPU-frei, und damit der Ausweg, wenn XTTS an einem Text reproduzierbar scheitert. Die Auswahl gibt es beim Anlegen **und** beim Bearbeiten; das Dropdown wird aus `GET /v1/admin/tts-engines` gefüllt (Registry seit v1.17 in `services/tts_engines.py`, gemeinsam mit der Hauptstimme — eine weitere Engine ist damit ein Listeneintrag). Da Piper nur eine Stimme hat, wird sein Audio für **alle** Stimmen abgelegt — der Abspielpfad (Cache-Matrix, `select_filler`) bleibt unverändert und muss die Engine nicht kennen. Ein Wechsel der Engine verwirft vorhandenes Audio (wie eine Textänderung).
- **Play-Button pro Stimme** in der Filler-Liste: `GET /v1/admin/fillers/{id}/audio?voice_id=…` liefert das gecachte WAV. Da ein `<audio src>` keinen `Authorization`-Header senden kann, holt das Panel die Datei per `fetch` mit Token und spielt sie als Blob-URL ab — kein Token in der URL. Stimmen ohne Audio erscheinen in derselben Spalte als Warn-Badge, fehlgeschlagene Generierungen sind damit auf einen Blick sichtbar.

**Saubere Vorgenerierung, Einzelstimmen, Trigger-Filter (NEU in v1.16):** Nutzer-Report: Vorgenerierte Filler endeten mit fremden Audiofetzen oder liefen in Zeitlupe, der Realtime-Talk nicht. Hauptursache: parallele XTTS-Synthesen verfälschen sich gegenseitig (Details unter 7. Bekannte Probleme). Umsetzung:
- **XTTS-Service rechnet immer nur eine Synthese** (Sperre in `XttsEngine`; weitere warten, nach `SYNTH_LOCK_TIMEOUT_S` = 120 s → 503). Der Stream-Endpunkt generiert in einem eigenen Thread und legt die Chunks in eine Queue: Bricht der Client ab (Barge-in), schließt Starlette den Generator nämlich nicht — die Sperre hinge sonst bis zum nächsten Garbage-Collector-Lauf. So ist sie spätestens mit dem Ende der Generierung frei. `set_device` wartet ebenfalls auf die laufende Synthese.
- **Neuer Endpunkt `POST /v1/synthesize/full`** (nicht streamend, komplettes WAV, optional `temperature`): XTTS' `inference()`-Qualitätspfad für die Vorgenerierung — GPT-Latents in einem vollständigen Durchlauf, Dekodierung am Stück, ohne Chunk-Nähte und gekapptes Ende des Streaming-Pfads. Der Realtime-Talk bleibt beim Streaming.
- **Orchestrator:** nur eine Filler-Generierung gleichzeitig (sonst stauten sich bei mehreren Klicks viele Synthesen vor XTTS und ein Realtime-Turn müsste hinter allen warten); Filler-Text bekommt ein sauberes Satzende (`...` im Satz → Komma, am Ende → Punkt, fehlendes Satzzeichen ergänzt — gespeichert bleibt der Originaltext); Stille vorn/hinten wird abgeschnitten (60 ms Vor-, 200 ms Nachlauf für leise Auslaute, weiche Blenden gegen Knackser); **Plausibilitätsprüfung** der Sprechdauer gegen die Textlänge (grob 12–16 Zeichen/s; > 0,4 s + Zeichen/9 gilt als Zeitlupe/angehängte Laute, < 0,1 s + Zeichen/40 als abgeschnitten) mit bis zu zwei Neuversuchen bei niedrigerer Temperatur (0,6 / 0,45); die erste plausible Aufnahme gewinnt, sonst die am wenigsten auffällige plus Hinweis im Panel. Neues Audio ersetzt das alte atomar (Temp-Datei + Umbenennen) und nur, wenn der Filler während der Generierung nicht umgetextet/umgestellt/gelöscht wurde.
- **Einzelne Stimme neu generieren:** `POST /v1/admin/fillers/{id}/generate?voice_id=…` — eine gelungene Stimme bleibt erhalten, wenn eine andere neu gewürfelt wird. Im Panel steht pro Stimme neben dem Play-Button ein ↻-Button; „Alle generieren" bleibt für den ganzen Filler.
- **Panel:** Während einer Generierung sind alle Generieren-Buttons gesperrt, der aktive zeigt einen Spinner, darüber steht ein Status; das Ergebnis erscheint als Meldung pro Stimme (fertig / Hinweis / Fehler) statt als `alert()`. **Filter-Chips „Alle · Trigger 1 · Trigger 2 …"** mit Anzahl über der Filler-Liste (Auswahl bleibt beim Neu-Rendern und im Browser gemerkt, neuer Filler landet vorausgewählt im gefilterten Trigger); unter „Alle" ist die Liste nach Trigger gruppiert.

**TTS-Engine der Hauptstimme wählbar + Probehören (NEU in v1.17):** Nutzerwunsch: das TTS-Modell im Orchestrator wechseln können, um neue Modelle (zuerst Breeze TTS 2) gegen XTTS zu testen. Umsetzung analog zum LLM-Panel:
- **Engine-Registry** (`services/tts_engines.py`): XTTS, Piper und Breeze TTS 2 sprechen dieselbe Schnittstelle — `stream(text, voice_id)` liefert `(Samplerate, PCM16-Chunk)`. Antwort-Pipeline, Filler-Generierung und Probehören kennen nur die Engine-ID; eine weitere Engine ist ein Client plus Registry-Eintrag. XTTS-Filler behalten ihren Qualitätspfad (v1.16); weitere Engines laufen bei der Filler-Generierung über ihren Stream, mit derselben Textaufbereitung, Trimmung und Plausibilitätsprüfung (ohne Temperatur-Neuversuche). Piper bekommt dafür einen Stream-Adapter (WAV → 24 kHz → 1-s-Chunks) und ist damit auch als Hauptstimme nutzbar (GPU-freier Notbetrieb).
- **Aktive Engine** server-weit in `app_settings.tts_engine` (Fallback `.env` `TTS_ENGINE`, Default `xtts`), gewählt in der neuen Panel-View **„Sprachausgabe"** (`GET /v1/admin/tts` mit Live-Status je Engine, `POST /v1/admin/tts/activate`). Aktivieren einer nicht erreichbaren Engine ist erlaubt, liefert aber eine Warnung.
- **Sicherheitsnetz:** Scheitert eine Nicht-XTTS-Engine vor dem ersten Chunk (Container gestoppt, Modell lädt noch, belegt), spricht XTTS die Antwort; nach dem ersten Chunk kein Wechsel mehr (sonst doppeltes Audio).
- **Probehören & Vergleich** (`POST /v1/admin/tts/preview`): Testsatz mit genau der gewählten Engine und Stimme — bewusst ohne Fallback — als WAV, dazu Zeit bis zum ersten Chunk (~1 s Audio), Gesamtdauer und Audiolänge als Header; das Panel zeigt daraus den Echtzeitfaktor. Grundlage für die TTS-Evaluation (Abschnitt 6) auf der echten Hardware.
- **Breeze TTS 2 als Test-Engine:** Open-Weight-Modell (Code Apache-2.0, Gewichte *BreezeBlue Research and Non-Commercial License* — privat ok). Offiziell **nur Englisch und Chinesisch**; ~7,7 GB VRAM (eager). Angebunden wird der **offizielle** Streaming-Server (`POST /v1/audio/speech`, Multipart, PCM16/24 kHz) in `tts-breeze/` — gepinnter Commit, ohne FlashAttention (baut nur ab Ampere, der Server rechnet ohnehin eager) — als **eigener Coolify-Deploy** (`docker-compose.breeze.yml`), weil er das Modell beim Start lädt und das VRAM dauerhaft belegt. Voice-Cloning braucht Sample **plus exaktes Transkript** (siehe Stimmen-Verwaltung); ohne Transkript spricht Breeze mit seiner eingebauten Stimme. Optionale Sprechanweisung („Voice Direction", `app_settings.breeze_instruction`, dann CFG 4 laut Breeze-README). Der Server nimmt genau einen Request zur Zeit (409) — der Client wartet bis ~5 s. Die Cloud-API von breezeblue.ai ist bewusst nicht angebunden (Privacy, Abschnitt 8).

**Breeze-Server-Adresse im Panel + Breeze-TTS-2.cpp (NEU in v1.18):** Nutzerwunsch nach der Frage, ob Breeze über LM Studio läuft (nein: LM Studio bietet keinen TTS-Endpunkt, und Breeze ist keine Architektur, die llama.cpp/MLX laden). Stattdessen als zweite Server-Variante **Breeze-TTS-2.cpp** (Community-Portierung auf ggml, Original `HoppouAI/Breeze-TTS-2.cpp`): quantisiert (Q8_0 ~4 GB statt ~7,7 GB VRAM), ohne bf16/FlashAttention (passt zur 2080 Ti), laut Projekt ~1,25x Echtzeit auf einer RTX 3060 — und laut Server-Code dieselbe Schnittstelle wie der offizielle Server (`POST /v1/audio/speech` mit `text`/`ref_audio`/`ref_text`/`instruction`/`cfg_scale`, PCM16 mit `X-Sample-Rate`, `GET /health`, 409 bei Belegung), der Orchestrator braucht keinen eigenen Client. Einziger Unterschied: ohne Sprechanweisung konditioniert sie auf ein eingebautes „Speak clearly and naturally.". Umsetzung:
- **Adresse im Panel** (Sprachausgabe → Breeze-Server, `app_settings.breeze_base_url`, leer = `.env` `BREEZE_BASE_URL`): Container-Name, IP:Port oder Domain; ohne Schema wird `http://` ergänzt, erlaubt sind http/https mit optionalem Port und Pfad-Präfix. Client, Status-Check und GPU-Panel nutzen dieselbe effektive Adresse; Vorschläge für beide Container und den nativen Server.
- **`tts-breeze-cpp/`**: Multi-Stage-Dockerfile — Build in `nvidia/cuda:12.8.1-devel-ubuntu24.04` (gepinnter Commit, `BREEZE_CUDA=ON`, statisch gelinkt, CUDA-Architekturen per Build-Arg, Default `61;75;86;89`), Laufzeit in `-runtime-ubuntu24.04`. Der Entrypoint (nur Standardbibliothek) lädt die GGUF der gewählten Quantisierung (`BREEZE_GGUF_QUANT`, Default `q8_0`, ohne die experimentellen `-dd`-Varianten) aus `HoppouAI/Breeze-TTS-2.cpp` — mit Fortsetzen abgebrochener Downloads — und startet `breeze-server` ohne WebSocket-Port. Eigener Deploy `docker-compose.breeze-cpp.yml`, Adresse `http://tts-breeze-cpp:7860`.
- **Nativ** (ohne Docker) per CMake mit Vulkan oder CUDA; Anleitung inkl. Bindung an eine interne Adresse (der Server hat keine Authentifizierung) und systemd-Dienst im README.

**Voice Engine Manager: TTS-Engine-Vertrag + Qwen3-TTS (NEU in v1.20):** Nach dem ersten Breeze-Test auf der VM (Stimme wenig ähnlich, Deutsch nicht getroffen — das Open-Weight-Modell kann offiziell kein Deutsch) der Nutzerwunsch: neue Sprachmodelle leicht einbinden, und ein Manager soll sie selbstständig starten, pausieren (entladen) und beenden. Erste Engine danach: **Qwen3-TTS**; Breeze bleibt vorerst unverändert (laut Hugging Face sollen weitere Sprachen folgen). Umsetzung:
- **Steuerung auf Anwendungsebene, nicht über Docker** (Begründung wie 4.2: kein Docker-Socket im Orchestrator, Coolify bleibt Herr der Container). Container starten/beenden bleibt ein Coolify-Deploy; „pausieren“ heißt **Modell-Prozess beenden** — ein Engine-Container ohne geladenes Modell belegt kein VRAM, nur etwas RAM. Für die Karte hat das denselben Effekt wie ein gestoppter Container, ohne Root-Rechte auf dem Host.
- **Engine-Vertrag v1** (`tts-engine-kit/README.md`, HTTP): `GET /v1/health`; `GET /v1/info` = Steckbrief (Name, Sprachen als ISO-Codes, Samplerate, braucht Sample/nutzt Transkript, Sprechanweisung, Streaming-Art, VRAM-Richtwert, Lizenz); `GET/POST /v1/device` mit `off`/`cpu`/`cuda[:N]` und Zuständen `preparing` (Gewichte laden) / `idle` / `loading` / `ready` / `off` / `error` (+ `detail`); `POST /v1/synthesize` (Multipart: Text, Sprache, Voice-ID, Referenz-WAV, Transkript, Anweisung, `load_timeout_s`) → PCM16-Stream mit `X-Sample-Rate`. Der erste Chunk entsteht vor den Antwort-Headern: Fehler vor dem ersten Audio kommen als HTTP-Status (400 ungültig, 409 belegt, 503 aus/lädt, 500 Engine-Fehler, jeweils mit `detail`) statt als abgerissener Stream (vgl. Breeze, v1.18.2).
- **Kit statt Copy-Paste:** Eine neue Engine ist eine Backend-Klasse (`info`, `prepare`, `load`, `prepare_voice`, `synthesize`) plus Dockerfile und Compose-Datei; den FastAPI-Server liefert das Kit. Das Modell läuft in einem **eigenen Prozess** (spawn): „Aus“ beendet ihn, das VRAM ist komplett frei; Abstürze (Segfault, OOM-Kill) meldet das Kit mit Exit-Code, der nächste Aufruf lädt neu; hängt das Modell länger als `ENGINE_CHUNK_TIMEOUT_S`, wird der Prozess beendet. Dazu: eine Synthese zur Zeit (weitere warten, dann 409), Referenz-Cache pro Sample + Transkript (SHA-256) solange das Modell geladen ist, Abbruch nach dem laufenden Chunk, wenn der Client trennt (Barge-in), Satz-Aufteilung für Modelle ohne Streaming, Sperrfrist nach Lade-/Downloadfehlern (ein Gerätewechsel versucht es sofort).
- **Orchestrator:** Vertrags-Engines stehen in der Tabelle `tts_contract_engines` (ID, Adresse, optionaler Name) und werden im Panel unter Sprachausgabe nur per ID + Adresse eingetragen, bearbeitet oder entfernt (`POST /v1/admin/tts/engines`, `DELETE /v1/admin/tts/engines/{id}`) — kein Code. Die Registry setzt sich aus den eingebauten Engines (XTTS, Piper, Breeze) und diesen Einträgen zusammen; Name, Sprachen und „braucht Sample“ kommen aus dem gecachten Steckbrief. Ein generischer `ContractClient` spricht alle: Referenz-Sample als mono PCM16 24 kHz (wie bei Breeze), Transkript und die Sprache der Stimme gehen mit. Jede Vertrags-Engine steht damit automatisch in der Engine-Tabelle, beim Probehören, bei den Fillern und im GPU-Panel (4.2); ihr Status kommt aus `/v1/device` („nicht geladen“, „lädt die Gewichte herunter“, „geladen auf cuda:0“ …). Qwen3 ist einmalig als `qwen3` → `http://tts-qwen3:8000` vorbelegt (`.env` `QWEN3_BASE_URL`), danach frei änderbar und entfernbar.
- **Aktivieren = Engine-Wechsel mit VRAM-Tausch:** `POST /v1/admin/tts/activate` lädt die Ziel-Engine auf ihre Karte (gespeicherte Zuweisung, sonst die vor dem Ausschalten, sonst die live gemeldete, sonst `cuda:0`) und entlädt vorher alle anderen ausschaltbaren Sprachausgaben, die **auf derselben Karte** geladen sind (XTTS, andere Vertrags-Engines); die andere Karte bleibt unberührt. Eine ausgeschaltete XTTS wird beim Aktivieren wieder eingeschaltet (v1.19: 409). Die Antwort nennt, was entladen/geladen wurde; das Panel zeigt es als Hinweis. Die aktive Hauptstimme lässt sich weiterhin nicht ausschalten.
- **Warten nur, wo es niemand merkt:** Live-Turns schicken `load_timeout_s=0` — ist das Modell nicht geladen (z. B. nach einem Container-Neustart), stößt der Request das Laden an, bekommt sofort 503, und die Rückfallebene spricht (XTTS, bzw. Piper, solange XTTS aus ist). Probehören und Filler-Generierung warten bis 240 s aufs Laden.
- **Qwen3-TTS** (`tts-qwen3/`, Apache-2.0, Paket `qwen-tts` 0.1.1): Base-Modelle 1.7B (Default, ~5 GB) oder 0.6B (~2,5 GB) per `QWEN3_MODEL`, Download beim ersten Start ins Volume `qwen3-models` (Marker-Datei, Fortsetzen nach Abbruch). Klont mit exaktem Transkript im ICL-Modus, ohne Transkript nur über den Stimm-Fingerabdruck (x-vector); Sprache per ISO-Code → Qwen-Sprachname (`de` → `german`, unbekannt → `auto`). dtype `auto`: bfloat16 ab Ampere, float16 auf Turing (2080 Ti), float32 auf der CPU, `QWEN3_DTYPE` überschreibt; NaN-Prüfung pro Satz mit Hinweis auf float32. Attention `sdpa` (FlashAttention 2 bräuchte Ampere). Das Python-Paket erzeugt ganze Äußerungen (Streaming nur über vLLM) → die Engine spricht **Satz für Satz**, erstes Audio nach dem ersten Satz. torch wie bei XTTS auf 2.12.1 gepinnt (läuft nachweislich mit dem Treiber der VM). Eigener Coolify-Deploy `docker-compose.qwen3.yml` (Base Directory `/`, Build-Context Repo-Root wegen des Kits); `ENGINE_PRELOAD=false`: Das Modell lädt erst beim Aktivieren bzw. Probehören, ein frischer Deploy belegt also kein VRAM neben XTTS.
...
- [~] **TTS-Evaluation** – Werkzeug steht (v1.17): Engine-Wechsel + Probehören mit Latenzmessung im Panel. Erste Test-Engine **Breeze TTS 2** eingebaut, als offizieller PyTorch-Server oder als Breeze-TTS-2.cpp (Q8_0, v1.18) (offiziell nur EN/ZH → deutscher Praxistest auf der VM offen, ebenso Tempo auf der RTX 2080 Ti). Erster Breeze-Test auf der VM (Stand v1.19): Stimme wenig ähnlich, Deutsch nicht getroffen — wie erwartet ohne offizielle Deutsch-Unterstützung. Seit v1.20 eingebaut: **Qwen3-TTS** (Apache-2.0, 0.6B/1.7B, 10 Sprachen inkl. Deutsch, Voice-Cloning mit Referenz + Transkript — [GitHub](https://github.com/QwenLM/Qwen3-TTS)) als erste Engine nach dem Engine-Vertrag (4.14) → Praxistest auf der VM offen (Klang, deutsche Aussprache, Tempo auf der 2080 Ti mit float16). Weitere Kandidaten lassen sich jetzt ohne Orchestrator-Code einbinden: Kokoro, StyleTTS2, Orpheus TTS; Breeze nach dem Vertrag, sobald das Open-Weight-Modell weitere Sprachen kann
...
### Coolify: Compose-Pfade sind Repo-Root-relativ (NEU in v1.7, korrigiert in v1.18.1)
**Symptom:** `failed to read dockerfile: open Dockerfile: no such file or directory` bzw. `unable to prepare context: path ".../orchestrator/<dienst>" not found` beim Deploy.
**Ursache:** Coolify führt `docker compose` mit `--project-directory <Checkout><Base Directory>` aus — relative `build.context`-Pfade löst Compose von dort auf, nicht vom Ordner der Compose-Datei. (Die Annahme aus v1.7, Coolify nehme immer das Repo-Root und das Base Directory steuere nur die Suche nach der Datei, war falsch. Die Repo-Root-relativen Pfade des Voice-Stacks funktionieren nur mit Base Directory `/`. Belegt im Deploy-Log der Breeze-Resource (v1.18.1): Base Directory `/orchestrator` → `--project-directory …/orchestrator`.)
**Status:** Gelöst per Konvention: Alle `build.context`-Pfade in den Compose-Dateien sind Repo-Root-relativ (`.`, `./stt-service`, `./tts-breeze-cpp`, …), jede Coolify-Resource bekommt **Base Directory `/`** und als Docker Compose Location den vollen Pfad (`/orchestrator/docker-compose.yml`, `/orchestrator/docker-compose.breeze.yml`, `/orchestrator/docker-compose.breeze-cpp.yml`). `env_file` ist optional markiert, weil `.env` gitignored ist.
...
### Vorgenerierte Filler: Fetzen am Ende, Zeitlupe (NEU in v1.16)
**Symptom:** XTTS-Filler enden mit Audiofetzen, die nicht zum Text gehören, oder sind zeitlich verzerrt bis zur Zeitlupe. Die Echtzeit-Sprachausgabe im Realtime-Talk ist sauber.
**Ursache:** XTTS-v2 ist nicht reentrant. coqui-tts legt die Text-Konditionierung einer Synthese **am Modell selbst** ab (`gpt.gpt_inference.cached_prefix_emb`) und liest sie bei **jedem** generierten Audio-Token wieder aus, u. a. für dessen Positions-Index. Das Panel erlaubte mehrere gleichzeitige „Audio generieren"-Klicks (ohne sichtbaren Fortschritt naheliegend) — die Synthesen verschränkten sich, und die laufende rechnete mit Text und Länge der neuen weiter: verschobenes Timing (Zeitlupe, Fetzen) oder ein negativer Positions-Index (`IndexError`, auf CUDA ein Device-Assert — sehr wahrscheinlich auch die „sporadisch scheiternden" Filler aus v1.15). Im Realtime-Talk laufen praktisch nie zwei Synthesen gleichzeitig. Nachgewiesen mit dem echten coqui-tts-0.27.5-Code (kleines Modell mit Zufallsgewichten, deterministische Dekodierung): kurzer Filler + längerer Parallel-Text → Absturz; langer Filler + kürzerer Parallel-Text → Audio weicht ab dem ersten Chunk ab; mit Sperre bit-identisch zur Einzel-Synthese. Dazu kommt XTTS' bekannte Schwäche bei sehr kurzen Sätzen (Laute nach dem Satzende), besonders ohne Satzzeichen am Ende.
**Lösung:** Siehe 4.14 „Saubere Vorgenerierung" (v1.16): Serialisierung im XTTS-Service und im Orchestrator, Qualitätspfad `/v1/synthesize/full`, sauberes Satzende, Plausibilitätsprüfung mit Neuversuch, Einzelstimmen neu generierbar.
**Status:** Gelöst (v1.16), Validierung mit dem echten Modell auf der VM offen. Vorhandene Filler einmal neu generieren — sie können noch aus der Zeit vor dem Fix stammen.
...
**Stand:** 2026-09-26
**Changelog:**
- v1.20 (2026-09-26): **Voice Engine Manager: TTS-Engine-Vertrag + Qwen3-TTS** (Nutzer-Report nach dem ersten Breeze-Test auf der VM: Der Absturz war ein VRAM-Überlauf, danach kam Audio, aber weder die Stimme noch die geschriebene Sprache getroffen; Nutzerwunsch: ein eigener Engine-Manager, über den sich neue Sprachmodelle leicht einbinden lassen und der sie selbstständig startet, pausiert/entlädt und beendet — erste Engine Qwen3-TTS, Breeze bleibt vorerst). Neu `tts-engine-kit/`: Engine-Vertrag v1 (HTTP: Steckbrief, Laden/Entladen, Synthese mit sauberen Fehlercodes) plus Server-Baukasten — Modell im eigenen Prozess („Aus“ = Prozess beenden, VRAM komplett frei), eine Synthese zur Zeit, Referenz-Cache, Abbruch, Satz-Aufteilung, Absturz- und Hänger-Erkennung. Neu `tts-qwen3/`: Qwen3-TTS-Base (1.7B/0.6B) als erste Vertrags-Engine, eigener Coolify-Deploy `docker-compose.qwen3.yml`. Orchestrator: Vertrags-Engines per ID + Adresse im Panel (Tabelle `tts_contract_engines`, Qwen3 vorbelegt), generischer Client, automatisch im GPU-Panel (Karte/Aus), Status aus `/v1/device`; **Aktivieren lädt die Engine und entlädt andere Sprachausgaben auf derselben Karte**, eine ausgeschaltete XTTS wird dabei wieder eingeschaltet; Live-Turns warten nie aufs Laden (Rückfallebene), Probehören/Filler bis 240 s. Container starten/beenden bleibt bewusst bei Coolify (kein Docker-Socket, 4.2) — pausieren = Modell entladen. README mit Installationsanleitung für Qwen3 und Anleitung „neue Engine einbinden“ (`tts-engine-kit/README.md`). 220 Orchestrator- (19 neue), 19 Kit- und 17 Qwen3-Tests grün, 3 davon gegen die echte `qwen-tts`-Bibliothek (Parameternamen, Sprachvalidierung, Referenz-Format); Panel-Ablauf (Probehören lädt, Aktivieren tauscht XTTS aus, zurück zu XTTS, Engine eintragen/entfernen) in Chromium gegen Fake-Backends durchgeklickt; Image-Layout per `uv`-Trockenlauf und `docker compose config` geprüft. **Offen:** Lauf mit echten Gewichten auf der VM (Hugging Face war aus der Entwicklungs-Sandbox gesperrt) — Klang, deutsche Aussprache, Tempo auf der 2080 Ti.
- v1.19 (2026-09-25): **XTTS im Admin-Panel ausschaltbar** (Nutzerfrage: XTTS läuft neben dem Breeze-Test weiter — wie stoppen, und geht das im Admin?). GPU-Panel: Zuweisung „Aus (Modell entladen)" für XTTS neben CPU/GPU 0/GPU 1, auf Anwendungsebene wie die Kartenwahl (4.2): Der XTTS-Dienst entlädt Modell und Latents, Synthesen bekommen 503 „ausgeschaltet"; der Orchestrator speichert und restauriert die Wahl, verweigert das Ausschalten, solange XTTS die aktive Hauptstimme ist, verweigert Aktivieren/Probehören/neue Filler, solange es aus ist, und nimmt Piper als Rückfallebene. Panel mit Rückfrage, Status „ausgeschaltet" unter Sprachausgabe, Rückfrage-Text nennt die tatsächliche Rückfallebene; Fake-XTTS kennt `/v1/device`. 201 Orchestrator- (8 neue) und 23 XTTS-Tests (2 neue) grün; Ablauf in Chromium gegen die Fake-Backends durchgeklickt.
- v1.18.2 (2026-09-25): **Breeze-Absturz beim Probehören verständlich + Sample-Format** (Nutzer-Report: Probehören mit Breeze-TTS-2.cpp und neuer Stimme → „peer closed connection without sending complete message body (incomplete chunked read)"). Ursache laut Server-Code: Breeze-TTS-2.cpp schickt die Antwort-Header vor dem Kodieren der Referenz; ein CUDA-Fehler danach (VRAM voll — Referenz-Kodierung und KV-Cache kommen pro Request zum Modell dazu —, oder `BREEZE_CUDA_ARCHS` ohne die Karte) beendet den Prozess mitten im Stream, Fehler innerhalb der Synthese fängt der Server sonst selbst ab. Probehören und Filler melden einen abgerissenen Stream jetzt als Absturz mit Log-Befehl und den zwei häufigsten Ursachen (`tts_engines.stream_abort_detail`, auch für `ReadError`). Außerdem geht das Voice-Sample als mono PCM16 24 kHz an Breeze: Der WAV-Leser von Breeze-TTS-2.cpp kennt nur 16/32 Bit und las 24-Bit-Samples als reine Stille (mit dem Original-C++-Code nachgewiesen). README mit Absturz-Diagnose. 193 Orchestrator-Tests grün (3 neue, davon einer gegen einen echten Socket-Server, der wie Breeze nach den Headern abbricht).
- v1.18.1 (2026-09-25): **Coolify-Anleitung für die Breeze-Deploys korrigiert** (Nutzer-Report: Deploy von `docker-compose.breeze-cpp.yml` brach ab mit `unable to prepare context: path ".../orchestrator/tts-breeze-cpp" not found`). Ursache: Die Anleitung nannte Base Directory `/orchestrator`, Coolify nimmt das Base Directory aber als `--project-directory`, und die Build-Contexts sind Repo-Root-relativ. Richtig ist Base Directory `/` + Compose Location `/orchestrator/docker-compose.breeze(-cpp).yml`, wie beim Voice-Stack; README (auch Abschnitt 3), Kopfkommentare der drei Compose-Dateien und der Bekannte-Probleme-Eintrag aus v1.7 entsprechend berichtigt. Neu in der Anleitung: Build arguments auf „Managed manually in Dockerfile", sonst schreibt Coolify jede Variable als `ARG` ins Dockerfile (Neubau bei jeder Änderung, `HF_TOKEN` in der Build-History), und `BREEZE_CUDA_ARCHS` muss die Karte aus `BREEZE_GPU` enthalten. Vorher gemeldet: „Failed to read the Docker Compose file from the repository." beim Anlegen der Resource — Sammel-Fehler, der in den aktuellen Coolify-Releases (bis v4.3.23/v4.4-rc.1) die Git-Meldung verschluckt; Coolify prüft vorab nur das Repo, nicht den Branch, ein abweichender Branch-Name scheitert erst beim Klonen. Kein Code geändert.
- v1.18 (2026-09-24): **Breeze-TTS-2.cpp als zweite Breeze-Variante + Server-Adresse im Panel** (Nutzerfrage: Breeze über LM Studio? — nein, LM Studio hat keinen TTS-Endpunkt; Nutzerwunsch: schlankere Variante + Eingabefeld für die Adresse). Neuer Ordner `tts-breeze-cpp/` (CUDA-Build der Community-Portierung, gepinnter Commit, statisch; GGUF-Download mit Fortsetzen) und `docker-compose.breeze-cpp.yml` — ~4 GB VRAM (Q8_0) statt ~7,7 GB, gleiche Schnittstelle, kein Orchestrator-Client nötig. Breeze-Server-Adresse im Panel einstellbar (`app_settings.breeze_base_url`, normalisiert/validiert), genutzt von Client, Status-Check und GPU-Panel. README mit Installationsanleitungen für den PyTorch-Container, den cpp-Container und den nativen Build (Vulkan, interne Bindung, systemd). 190 Orchestrator-Tests grün (13 neue). CUDA-Build des Containers mit dem Toolkit aus `nvidia/cuda:12.8.1-devel-ubuntu24.04` real durchgeführt (~29 min für vier Architekturen auf 4 Kernen, Maschinencode für sm_61/75/86/89) und das Binary im entpackten `-runtime`-Image gestartet (alle Bibliotheken gefunden; `libgomp1`/`python3` fehlen dort und werden nachinstalliert); nativer Vulkan-Build nach Anleitung getestet (braucht zusätzlich `spirv-headers`); Entrypoint gegen einen Hugging-Face-Fake getestet (Auswahl, Fortsetzen, Token). Offen: Lauf mit echten Gewichten auf der VM.
- v1.17.1 (2026-09-24): **Breeze „nicht erreichbar“ verständlich** (Nutzer-Report: Aktivieren von Breeze zeigte „[Errno -2] Name or service not known“ und wirkte wie ein Programmfehler — tatsächlich lief der Breeze-Deploy noch nicht). Der Status-Check übersetzt die Ursache jetzt in den nächsten Schritt: unbekannter Name → „Container nicht gefunden“ + Hinweis auf den eigenen Deploy (`docker-compose.breeze.yml`), abgelehnte Verbindung → Dienst startet noch/abgestürzt + `docker logs`, Timeout. Das Panel fragt vor dem Aktivieren einer nicht erreichbaren Engine nach (statt hinterher zu warnen). README mit Diagnose-Befehlen. 177 Orchestrator-Tests grün (3 neue); Szenario mit echtem DNS-Fehler im Panel (Chromium) nachgestellt.
- v1.17 (2026-09-24): **TTS-Engine wechselbar + Breeze TTS 2 als Test-Engine** (Nutzerwunsch: TTS-Modell im Orchestrator wechseln, zuerst Breeze TTS 2 testen). Gemeinsame Engine-Registry (`services/tts_engines.py`) für Hauptstimme, Filler und Probehören — XTTS, Piper (neu auch als Hauptstimme) und Breeze. Aktive Engine server-weit in `app_settings.tts_engine` (Fallback `.env` `TTS_ENGINE`), gewählt in der neuen Panel-View „Sprachausgabe" mit Live-Status; fällt eine Nicht-XTTS-Engine vor dem ersten Chunk aus, spricht XTTS. Probehören pro Engine + Stimme ohne Fallback mit Zeit bis zum ersten Chunk, Gesamtdauer und Echtzeitfaktor. Breeze: offizieller Streaming-Server in `tts-breeze/` (gepinnter Commit, ohne FlashAttention) als eigener Coolify-Deploy (`docker-compose.breeze.yml`, ~7,7 GB VRAM ab Start, Karte per `BREEZE_GPU`); Voice-Cloning mit Sample + exaktem Transkript (`voices.sample_text`, beim Upload per Whisper vorgeschlagen, editier- und neu transkribierbar), optionale Sprechanweisung. **Wichtig:** Das Open-Weight-Modell spricht offiziell nur Englisch/Chinesisch. GPU-Panel führt Breeze als Info-Eintrag. Fake-Backends um Breeze erweitert. 174 Orchestrator-Tests grün (26 neue); Engine-Wechsel, Transkript-Vorschlag, Breeze-Multipart (Klon und eingebaute Stimme) und Sprach-Turn zusätzlich live gegen Fake-Backends und das Panel in Chromium geprüft; Import des offiziellen Breeze-Servers mit der Image-Paketliste (ohne flash-attn) verifiziert. **Offen:** Test auf der VM (echte Gewichte, deutsche Aussprache, Tempo auf Turing).
- v1.16 (2026-09-23): **Filler sauber vorgenerieren + Einzelstimmen + Trigger-Filter** (Nutzer-Report: Filler mit fremden Audiofetzen am Ende bzw. in Zeitlupe, Realtime-Talk sauber). Hauptursache gefunden und mit dem echten coqui-tts-Code nachgewiesen: parallele XTTS-Synthesen überschreiben sich den am Modell abgelegten Text-Präfix (`cached_prefix_emb`) → verschobenes Timing oder `IndexError` (siehe Bekannte Probleme). XTTS-Service: immer nur eine Synthese (Stream über Producer-Thread, damit ein Client-Abbruch die Sperre nicht blockiert; 503 nach 120 s Wartezeit), neuer Qualitätspfad `POST /v1/synthesize/full`. Orchestrator: eine Filler-Generierung zur Zeit, sauberes Satzende, Stille trimmen, Plausibilitätsprüfung der Sprechdauer mit bis zu zwei Neuversuchen, atomares Schreiben, `?voice_id=` zum Neu-Generieren einzelner Stimmen. Panel: ↻ pro Stimme, Sperre + Spinner während der Generierung, Ergebnis-Meldung statt `alert()`, Filter-Chips „Alle · Trigger …" über der Filler-Liste. 148 Orchestrator- und 21 XTTS-Tests grün.
- v1.15 (2026-09-23): **Filler: Engine-Wahl + Probehören** (Nutzer-Report: XTTS scheitert bei einzelnen Fillern sporadisch, fällt aber erst im Realtime-Talk auf). Pro Filler ist die TTS-Engine wählbar (`xtts` = Nutzerstimme, `piper` = feste Stimme, robust und GPU-frei) — beim Anlegen und beim Bearbeiten, Dropdown aus `GET /v1/admin/tts-engines`; Piper-Audio gilt für alle Stimmen, der Abspielpfad bleibt unverändert. Engine-Wechsel verwirft altes Audio. Neuer Play-Button pro Stimme in der Filler-Liste (`GET /v1/admin/fillers/{id}/audio?voice_id=…`, per fetch mit Token als Blob abgespielt — kein Token in der URL); Stimmen ohne Audio stehen als Warn-Badge daneben. 120 Tests grün.
...
- v1.12.5 (2026-07-16): **Interaktive Karten** (Nutzer-Test: „Karte, in der ich Flüge raussuchen kann" — das Modell verneinte die Fähigkeit). `show_card` mit neuem `request`-Feld (Funktionsbeschreibung als Design-Brief an `code-card`, `data` darf dann leer sein); Tool-Beschreibung sagt ausdrücklich, dass Karten interaktive Mini-Tools sein können (Formulare, Buttons, Such-Links). Sandbox um `allow-forms`/`allow-popups`(`-to-escape-sandbox`) erweitert — weiterhin ohne same-origin; `code-card`-Auftrag erlaubt Inline-JS, verbietet externe Ressourcen/fetch, externe Aktionen als URL im neuen Tab. In Chromium verifiziert: Eingabe → Klick → neuer Tab mit gebauter Google-Flights-URL. 88 Tests grün.
