...
- **Breeze TTS 2** (Test-Engine, v1.17) nutzt den offiziellen Streaming-Server, der sein Modell beim Containerstart auf eine feste Karte lädt und keinen Laufzeit-Wechsel kennt — Karte per `BREEZE_GPU` in `docker-compose.breeze.yml`. Belegt ~7,7 GB, solange der Container läuft; deshalb eigener Deploy, der nur während Tests läuft.
...
- [~] **TTS-Evaluation** – Werkzeug steht (v1.17): Engine-Wechsel + Probehören mit Latenzmessung im Panel. Erste Test-Engine **Breeze TTS 2** eingebaut (offiziell nur EN/ZH → deutscher Praxistest auf der VM offen, ebenso Tempo auf der RTX 2080 Ti ohne natives bf16). Nächster Kandidat mit offizieller Deutsch-Unterstützung: **Qwen3-TTS** (Apache-2.0, 0.6B/1.7B, 10 Sprachen inkl. Deutsch, Voice-Cloning mit Referenz + Transkript, Streaming — [GitHub](https://github.com/QwenLM/Qwen3-TTS)); weiterhin offen: Kokoro, StyleTTS2, Orpheus TTS
