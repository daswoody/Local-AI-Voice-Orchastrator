...
- **Engine pro Filler wählbar** (`fillers.engine`, additive Migration, Default `xtts`): `xtts` spricht in der Nutzerstimme (kein Stimmbruch, braucht Voice-Sample und VRAM), `piper` nutzt die feste Piper-Stimme — robust, GPU-frei, und damit der Ausweg, wenn XTTS an einem Text reproduzierbar scheitert. Die Auswahl gibt es beim Anlegen **und** beim Bearbeiten; das Dropdown wird aus `GET /v1/admin/tts-engines` gefüllt (Registry in `filler_service.TTS_ENGINES`, eine dritte Engine ist damit ein Listeneintrag). Da Piper nur eine Stimme hat, wird sein Audio für **alle** Stimmen abgelegt — der Abspielpfad (Cache-Matrix, `select_filler`) bleibt unverändert und muss die Engine nicht kennen. Ein Wechsel der Engine verwirft vorhandenes Audio (wie eine Textänderung).
